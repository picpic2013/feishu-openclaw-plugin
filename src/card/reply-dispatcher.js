...
export function createFeishuReplyDispatcher(params) {
    ...
    //   streaming === true + groupStreaming === true → group=streaming (新行为)
    //   streaming 未设置    → 全 static (新默认)
    const streamingFlag = feishuCfg?.streaming;
    const groupStreamingFlag = feishuCfg?.groupStreaming === true; // [Custom] 群聊流式开关
    const replyMode = effectiveReplyMode === "auto"
        ? streamingFlag === true
            ? chatType === "group"
                ? (groupStreamingFlag ? "streaming" : "static") // [Custom] 群聊流式支持
                : "streaming"
            : "static"
        : effectiveReplyMode;
    ...
    params.runtime.log?.(`feishu[${account.accountId}]: replyMode=${effectiveReplyMode} -> ${replyMode} (chatType=${chatType}), groupStreaming=${groupStreamingFlag}, multiMessage=${useMultiMessageMode}`);
