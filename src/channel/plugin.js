    ...
    configSchema: {
        schema: {
            ...
            properties: {
                ...
                // [Custom] 群聊流式输出开关
                groupStreaming: { type: "boolean" },
                // [Custom] 多消息模式：每个阶段发送独立消息，每条消息内部流式输出
                multiMessageStreaming: { type: "boolean" },
                // [Custom] 流式更新节流间隔（毫秒），默认 1500ms，不指定则保持原行为
                streamingThrottleMs: { type: "integer", minimum: 500 },
                // [Custom] 流式更新长空闲后批量延迟（毫秒），默认 300ms，不指定则保持原行为
                streamingBatchMs: { type: "integer", minimum: 100 },
